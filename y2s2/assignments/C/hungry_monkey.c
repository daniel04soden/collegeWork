 ...
 * 0 otherwise.
 */
static int update_field(int field[][FIELD_WIDTH], int monkey_col,
                        struct prng_s *prng) {
  int i, j;
  int dice;
  int ret = 0;
...
int main() {
  ...
  while (step < 100 + FIELD_HEIGHT) {
    ...
    if (monkey_col < 0)
      monkey_col = 0;
    if (monkey_col >= FIELD_WIDTH)
      monkey_col = FIELD_WIDTH - 1;

    sprintf(position, "%d: %d: %d", step, monkey_col, treats_eaten);
  ...
  clear();
  char final[50];
  sprintf(final, "Game over! Score: %d\n", treats_eaten);
  mvwaddstr(stdscr, FIELD_HEIGHT >> 1, FIELD_WIDTH >> 2, final);
  getch();
