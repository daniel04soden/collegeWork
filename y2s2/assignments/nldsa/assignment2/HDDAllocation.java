...
public class HDDAllocation {
   ...
   * @param hddIdx
   * 
   *               Index of hard drive we are currently evaluating, ie if the file
   *               at file idx fits
   *               on this hard drive.
   *               This is achieved via summing up our current allocation (could
   *               be optimised through caching of some sort??)
   *               And checking if after adding this new allocation, is it still
   *               less than or equal to the hdd size
   * 
   * @return boolean of whether we can allocate or not (if the list with the newly
   *         allocated file works)
   */
  ...
  /**
   * @param fileIdx
   *                This function takes in a fileIdx, generally starting at 0 and
   *                from there
   *                we look for the solution which occurs when we've hit the last
   *                file.
   * 
   *                After checking this we loop over our hard drives and follow
   *                the backtracking
   *                steps of making a choice, evaluating the constraints (fitting
   *                into the hard drive)
   *                and if it does not fit those constraints, we backtrack by
   *                setting the res at that file
   *                index back to -1
   * 
   *                This function makes use of recursion to check between each
   *                file and their different hard drive
   *                fittings. When verifying if it fits our constraints, we
   *                recurse onto the next file index. If we can't find any
   *                solution for that file to a particular hard drive, we return
   *                null
   * @return res
   */
