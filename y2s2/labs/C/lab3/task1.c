...
int main() {
  char temp_array[30]; // a temporary array that is long enough to hold user
                       // input
  ...
  while (true) {
    ...
    bool isAlpha = (userInput >= 'a' && userInput <= 'z') ||
  ...
  char duplicatedArrayThree[i * 3 + 1]; // plus 1 to accommodate the \0 at the
                                        // end of the string
  for (int j = 0; j < i; j++) {
    duplicatedArrayThree[j] = temp_array[i];
    duplicatedArrayThree[i * 3 - 1 - j] = temp_array[j];
  }
  duplicatedArrayThree[i * 3] =
      '\0'; // need to add NULL for this to be a valid string
