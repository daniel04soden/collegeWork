public class QuickSort {
  ...
  private static void swapArrayVariables(int[] myArray, int v1, int v2) { // Method added to just swap the values in the
                                                                          // array
  ...
  private static void quicksort(int[] A, int p, int r) {
    ...
    if (p < r) { // if p is less than r then
      ...
      quicksort(A, p, q); // Recursively call this funciton with the values p and q as start and end
                          // points ie up to partition
